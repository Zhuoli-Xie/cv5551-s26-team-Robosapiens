...
def grounded_dino_detect(image, text_prompts, dino_model, box_thresholds):
    """Run GroundingDINO detection only (no SAM). Returns detections object."""
    assert len(image.shape) == 3
    text_threshold = 0.25
...
def sam_segment_detections(image, sam_model, detections, text_prompts, max_inst=None):
    """Run SAM segmentation on pre-detected boxes. Returns mask, labels, confidence."""
    ...
    if max_inst is not None and len(detections.mask) > max_inst:
        top_indices = np.argsort(detections.confidence)[::-1][:max_inst]
        detections.mask = detections.mask[top_indices]
...
def grounded_instance_sam_new_ver(image,
    ...
    # keep only top max_inst instances (by confidence) if specified
    if max_inst is not None and len(detections.mask) > max_inst:
        top_indices = np.argsort(detections.confidence)[::-1][:max_inst]
        detections.mask = detections.mask[top_indices]
        detections.confidence = detections.confidence[top_indices]
