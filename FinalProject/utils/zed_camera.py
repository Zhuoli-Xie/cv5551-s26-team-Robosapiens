...
class ZedCamera:
    ...
    def __init__(self, resolution=sl.RESOLUTION.HD2K, fps=15, exposure=20, serial_number=None):
    ...
    def get_synced_frame(self):
        """
        Return (image, point_cloud) from the same grab. Prefer this over separate
        .image and .point_cloud calls to avoid mask / depth misalignment.
        """
        with self._lock:
            if self._image is None or self._point_cloud is None:
                return None, None
            return self._image.copy(), self._point_cloud.copy()
