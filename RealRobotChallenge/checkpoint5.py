...
def main():
    ...
    try:
        ...
        for cube_name in stacking_order:
            result = cube_pose_detector.get_transforms(cv_image, cube_name)
            if result is None:
        ...
        if key == ord('k'):
            ...
            # Build order is reversed: place bottom first, then middle on bottom, then top on middle
            # Step 1: Stack middle cube onto bottom cube
            bottom_cube = stacking_order[2]
            middle_cube = stacking_order[1]
            top_cube = stacking_order[0]

            # Pick middle cube and place on bottom cube
            t_robot_middle, _ = cube_poses[middle_cube]
            t_robot_bottom, _ = cube_poses[bottom_cube]

            grasp_cube(arm, t_robot_middle)

            t_stack = t_robot_bottom.copy()
            t_stack[2, 3] += STACK_HEIGHT
            place_cube(arm, t_stack)

            # Step 2: Pick top cube and place on the stack (now 2 cubes high)
