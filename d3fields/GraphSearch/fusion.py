import os
import sys
# Ensure project root is on sys.path so that `utils.*` imports work
...
# XMem imports are deferred to __init__ (only needed when skip_xmem=False)
from utils.grounded_sam import grounded_instance_sam_new_ver, grounded_dino_detect, sam_segment_detections
...
class Fusion():
    def __init__(self, num_cam, feat_backbone='dinov2', device='cuda:0', dtype=torch.float32, skip_xmem=False):
        self.device = device
        self.dtype = dtype
        ...
        if self.feat_backbone == 'dinov2':
            ...
            # Keep on CPU; move to GPU only during forward pass to save VRAM
        ...
        sam_checkpoint = os.path.join(curr_path, 'ckpts/sam_vit_b_01ec64.pth')
        if not os.path.exists(sam_checkpoint):
            ...
            os.system(f'mv sam_vit_b_01ec64.pth {ckpts_dir}')
        self.ground_dino_model = GroundingDINOModel(config_file, grounded_checkpoint, device='cpu')

        from segment_anything import sam_model_registry
        self.sam_model = SamPredictor(sam_model_registry['vit_b'](checkpoint=sam_checkpoint))
        # Keep SAM on CPU to save VRAM; moved to GPU on demand
        self.sam_model.model = self.sam_model.model.cpu()
        
        # load XMem model (optional — only needed for temporal tracking)
        if not skip_xmem:
            ...
            print('Skipping XMem model loading to save GPU memory.')
            self.xmem_first_mask_loaded = False
            self.track_ids = [0]
    ...
    def extract_dinov2_features(self, imgs, params):
        ...
        with torch.no_grad():
            # Move DINOv2 to GPU for inference, then back to CPU to free VRAM
    ...
    def _move_detection_models_to_gpu(self):
        self.ground_dino_model.model.to(self.device)
        self.ground_dino_model.device = self.device
        self.sam_model.model.to(self.device)

    def _move_detection_models_to_cpu(self):
        self.ground_dino_model.model.cpu()
        self.ground_dino_model.device = 'cpu'
        self.sam_model.model.cpu()
        torch.cuda.empty_cache()

    def text_queries_for_inst_mask_no_track(self, queries, thresholds, boundaries, merge_all = False, expected_labels=None, robot_pcd=None, max_inst=None):
        # Temporarily offload observation tensors to CPU to free GPU for detection models
        gpu_keys = [k for k, v in self.curr_obs_torch.items() if isinstance(v, torch.Tensor) and v.is_cuda]
        for k in gpu_keys:
            self.curr_obs_torch[k] = self.curr_obs_torch[k].cpu()
        torch.cuda.empty_cache()

        # Phase 1: GroundingDINO detection (on GPU), then offload
        self.ground_dino_model.model.to(self.device)
        self.ground_dino_model.device = self.device
        all_detections = []
        for i in range(self.num_cam):
            det = grounded_dino_detect(self.curr_obs_torch['color'][i], queries, self.ground_dino_model, thresholds)
            all_detections.append(det)
        self.ground_dino_model.model.cpu()
        self.ground_dino_model.device = 'cpu'
        torch.cuda.empty_cache()

        # Phase 2: SAM segmentation (on GPU, autocast handles float16), then offload
        self.sam_model.model.to(self.device)
        masks = []
        labels = []
        mask_confs = []
        for i in range(self.num_cam):
            mask, label, mask_conf = sam_segment_detections(self.curr_obs_torch['color'][i], self.sam_model, all_detections[i], queries, max_inst=max_inst)
        ...
        # Restore observation tensors back to GPU
        for k in gpu_keys:
            if k in self.curr_obs_torch and isinstance(self.curr_obs_torch[k], torch.Tensor):
                self.curr_obs_torch[k] = self.curr_obs_torch[k].to(self.device)
    ...
    def text_queries_for_inst_mask(self, queries, thresholds, boundaries, use_sam=False, merge_all = False, expected_labels=None, robot_pcd=None):
        ...
        if not self.xmem_first_mask_loaded:
            self._move_detection_models_to_gpu()
            masks = []
            labels = []
            mask_confs = []
            for i in range(self.num_cam):
            ...
            print(self.curr_obs_torch['mask_label'])
            self.align_instance_mask_v3(queries, boundaries, expected_labels)
            self._move_detection_models_to_cpu()
    ...
    def create_instance_mask_mesh(self, vertices, triangles, res):
        ...
        for k in res.keys():
            if k.startswith('mask'):
                ...
                vertices_color = trimesh.visual.interpolate(mask / num_instance, color_map=plt.get_cmap('jet'))
                mask_meshes.append(trimesh.Trimesh(vertices=vertices, faces=triangles[..., ::-1], vertex_colors=vertices_color))
        return mask_meshes
